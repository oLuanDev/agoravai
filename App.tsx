...
import { USERS, INITIAL_CANDIDATES, INITIAL_TALENT_POOL, INITIAL_MESSAGES } from './constants';
...
function App() {
  ...
  const handleUpdateJob = async (updatedJob: Job) => {
    try {
      ...
      logHistory('UPDATE_JOB', `Atualizou a vaga '${newJob.title}'.`);
    } catch (error) {
  ...
  const handleArchiveJob = async (jobId: number) => {
    ...
    try {
      ...
      logHistory('ARCHIVE_JOB', `Arquivou a vaga '${jobToArchive.title}'.`);
    } catch (error) {
  ...
  const handleRestoreJob = async (jobId: number) => {
    ...
    try {
      ...
      logHistory('RESTORE_JOB', `Restaurou a vaga '${jobToRestore.title}'.`);
    } catch (error) {
      console.error("Failed to restore job:", error);
    }
  };

  const handlePermanentDeleteJob = async (jobId: number) => {
    const jobToDelete = jobs.find(j => j.id === jobId);
    if (!jobToDelete) return;
    try {
      await deleteJob(jobId);
      setJobs(prev => prev.filter(j => j.id !== jobId));
      logHistory('DELETE_JOB', `Excluiu permanentemente a vaga '${jobToDelete.title}'.`);
    } catch (error) {
